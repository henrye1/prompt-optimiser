...
The **browser authenticates directly against Supabase** (email/password) and sends the JWT as a `Bearer` token to the backend. `requireAuth()` (`backend/src/middleware/auth.ts`) verifies the token and attaches a **per-request Supabase client bound to that token** (`req.supabase`) so `auth.uid()` resolves and **RLS enforces ownership/publish/soft-delete in Postgres**. Always use `req.supabase` for user-scoped data access. The service-role client (`createServiceClient`) bypasses RLS and is for admin/seed only — never to serve a user request, with one sanctioned exception (see below). Background work (run execution) builds its own token-bound client via `createUserClient(req.accessToken)` so it outlives the request but still honors RLS.

**Sanctioned service-role carve-out — authorize-then-fetch.** The file download route (`GET /api/files/:id/download` in `runs.router.ts`) is the one place that serves a user request with the service-role client. It is only safe because authorization happens *first* on the caller's token-bound client: `getDownloadableFile(req.supabase, …)` returns the row only when RLS permits (owner, or a published run via `run_file_select`). Storage objects live under the owner's folder (`<auth.uid()>/…`, enforced by `0003_storage.sql`), so a published-run *viewer* can't read another user's bytes with their own token — hence the service-role client fetches the bytes *after* the RLS check passes. Any future service-role-in-a-request use must follow this same shape: **authorize on the user client, then act on the service client** — never act on the service client without an RLS-backed authorization gate.
